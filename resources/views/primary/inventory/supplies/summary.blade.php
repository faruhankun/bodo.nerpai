        ...
        <div class="max-w-7xl my-10 mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-white">
                    ...
                    @php 
                        $data = $supplies_spaces;
                        $supplies_per_space = $supplies_spaces->groupBy('space_id');
                    @endphp
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="border px-4 py-2">{{ 
                                                number_format(
                                                    $data->sum(function ($supply) {
                                                        return $supply->balance * $supply->cost_per_unit;
                                                    }), 
                                                    2) 
                                            }}</td>
                                    </tr>
                    ...
                    <div class="grid grid-cols-3 md:grid-cols-3 gap-6">
                        @foreach($supplies_per_space as $space_id => $space_data)
                            <x-div.box-show title="Space: {{ $space_id }}">
                                <table class="table-auto w-full mt-4">
                                    <thead>
                                        <tr>
                                            <th class="px-4 py-2">Keterangan</th>
                                    ...
                                    <tbody>
                                        <tr>
                                            <td class="border px-4 py-2">Inventory Count</td>
                                            <td class="border px-4 py-2">{{ $space_data->count() }}</td>
                                        </tr>
                                        <tr>
                                            <td class="border px-4 py-2">Inventory Value</td>
                                            <td class="border px-4 py-2">{{ 
                                                number_format(
                                                    $space_data->sum(function ($supply) {
                                                        return $supply->balance * $supply->cost_per_unit;
                                                    }), 
                                                    2) 
                                            }}</td>
