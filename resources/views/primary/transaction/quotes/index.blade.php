@php
    ...
    if(($user->can('space.quotes.po') OR $user->can('space.quotes.so')) || $space_role == 'owner'){
        $model_type_option['all'] = 'Semua Penawaran';   

        if($model_type_select == null)
            $model_type_select = 'all';
    }

    if($user->can('space.quotes.po') || $space_role == 'owner'){
    ...
    if($user->can('space.quotes.so') || $space_role == 'owner'){
...
<x-crud.index-basic header="Penawaran" model="quotes" table_id="indexTable" 
                    :thead="['Date', 'Number', 'Team', 'Kontak', 'Description', 'SKU', 'Status', 'Actions']">
    <x-slot name="buttons">
        @include('primary.transaction.quotes.create')
    ...
    <x-slot name="filters">
        <!-- export import  -->
        @if($user->can('space.quotes.exim') || $space_role == 'owner')
        <x-crud.exim-csv route_import="{{ route('quotes.exim', ['query' => 'import']) }}" route_template="{{ route('quotes.exim', ['query' => 'importTemplate']) }}">
    ...
    <x-slot name="modals">
        @include('primary.transaction.quotes.showjs')
    </x-slot>
</x-crud.index-basic>
...
<script>
    $(document).ready(function() {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        return `<a href="quotes/${row.id}" target="_blank">${
                                    data
                                }</a>`;
                    }
                },
        ...
        $('#exportVisibleBtn').on('click', function(e) {
            ...
            let exportUrl = '{{ route("quotes.exim") }}' + '?query=export' 
    ...
    function showjs(data) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                let modal_edit_link = '/quotes/' + parsed.id + '/edit';
